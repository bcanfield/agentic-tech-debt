...
Cursor adapter. Mirrors the Codex hook but emits Cursor's sessionStart envelope
(`additional_context`, snake_case) instead of `hookSpecificOutput.additionalContext`.
Reads/writes AGENTS.md (Cursor's charter file) and resolves the cache from one
deterministic base so the skill shell env agrees with the hook subprocess.
...
no existing ADR directory, the inject asks Cursor to detect it semantically and
...
# Cursor's sessionStart reads `additional_context` (snake_case) and injects it
# into the conversation's initial system context — the same channel Claude/Codex
# get via hookSpecificOutput.additionalContext, just a flatter envelope.
def emit(context):
...
# the ADR dir Cursor should write new ADRs into; Discipline 3 names the
...
def disciplines_text(adr_dir, registry_dir):
    return (
        ...
        '`debt-ops-add` skill immediately — no prompt. This is judgment, not a marker scan: a '
        ...
        'with a payoff trigger *is* deliberate debt — when you write one, also use the `debt-ops-add` '
        'skill so the registry entry mirrors the ADR (don\'t conclude "no markers, no debt").\n\n'
