#!/usr/bin/env python3
"""debt-ops beforeSubmitPrompt hook: handle 'drop A,B' / 'drop all' shorthand.

Cursor adapter. When the user types `drop A`, `drop A,C`, or `drop all` as the
entire prompt, this hook deletes the matching entries from the most recent batch
and blocks the submission (`continue: false`) with a one-line confirmation in
`user_message` — no agent turn consumed. This is the channel Copilot lacks
(its userPromptSubmitted output isn't processed), so drops are manual there;
on Cursor they work like Claude/Codex.

Other "drop" forms ("drop it", "drop foo-slug") aren't matched and fall through
to the agent's normal handling per the add skill.
"""
...
# Cursor's beforeSubmitPrompt blocks the submission with `continue: false` and
# shows `user_message` to the developer — no agent turn consumed.
def emit_block(reason):
    sys.stdout.write(json.dumps({"continue": False, "user_message": reason}) + "\n")
...
def main():
    ...
    # Guard: don't intercept "drop it" — let the agent handle it conversationally.
    ...
    # Deterministic cache base shared with the hooks + skills. Override with
    # DEBT_OPS_CACHE; default ~/.cache/debt-ops (the open-standard group's base).
    cache_base = Path(os.environ.get("DEBT_OPS_CACHE") or (Path.home() / ".cache" / "debt-ops"))
    ...
    if not deleted:
        # Nothing actually deleted — pass through to the agent so they can ask why.
        return 0
