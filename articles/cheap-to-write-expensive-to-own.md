...
![GitClear stat card. From 2020 to 2024, the share of changed lines that were copy-pasted rose from 8.3 to 12.3 percent, while moved or refactored lines fell from 24.1 to 9.5 percent. 2024 was the first year GitClear measured copy-pasted code beating refactored code, and duplicated blocks of five or more lines rose eightfold that year. A note flags it as vendor research and a correlation with AI adoption, not proven causation.](/cheap-to-write-expensive-to-own.anchor.png)
...
![Before-and-after card from the repo's ADR log. ADR 0011 scheduled extracting a shared module once a third adapter landed; ADR 0014 superseded it, keeping four self-contained copies synced by hand to trade a higher cost to write for a lower cost to own. A note adds that most of the decisions a coding agent makes never get an ADR, so their reasoning is gone by the next session.](/cheap-to-write-expensive-to-own.adr.png)
