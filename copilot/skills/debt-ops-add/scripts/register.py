...
Portable Agent Skill (Agent Skills open standard). Writing through this helper
keeps the agent's mid-turn footprint to one line — the helper's own stdout — and
assigns a short letter (A, B, C, ...) per entry so the user can later say
`drop A`. Logic is kept byte-identical to the adapter copies so the planned
`_common.py` extraction (ADR 0011 / packaging plan Phase E) is a clean lift.
"""
...
# Single deterministic cache base, shared verbatim with any hook adapter (ADR 0011).
