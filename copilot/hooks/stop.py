...
Copilot adapter. Wired to Copilot's `agentStop` hook — its output contract
(`decision: "block"` + `reason`) matches Claude/Codex's Stop, so the safety net
ports cleanly. Fires at the end of every turn. Counts newly-added marker lines
(TODO/FIXME/HACK/XXX) in the working tree vs newly-added entries under the
registry dir. If markers > registrations, nudges Copilot on the next turn via a
...
# Plugin-source prefixes are always excluded (this monorepo hosts every
# adapter); debt-registry and ADR prefixes are looked up per-repo from the
# cache files written by session-start.py. See build_excluded_prefixes below.
STATIC_EXCLUDED_PREFIXES = ("claude-code/", "codex/", "copilot/", "cursor/", "skills/")
...
def main():
    ...
    # Stage 2: no markers and no registrations, but code changed — let Copilot
    ...
    # Copilot delta: no batch rotation. Claude/Codex rotate current-turn.txt into
    # last-batch.txt here so the next turn's UserPromptSubmit `drop A` resolves the
    # just-finished turn. Copilot has no userPromptSubmitted drop hook (its output
    # isn't processed), so drops are manual and read current-turn.txt directly —
    # rotating it away would break that. Leave it in place.
