...
> A practical reference for every piece of a Claude Code plugin: what each piece is, when to reach for it, and a real GitHub example you can read or install today.

A **plugin** is a self-contained directory that extends Claude Code with custom functionality. Plugins are distributed through marketplaces, copied into a local cache on install (`~/.claude/plugins/cache`), and discovered by Claude Code automatically. Anything you can do with `.claude/` standalone configuration you can do inside a plugin — but plugins are versioned, namespaced, and shareable.

Use **standalone `.claude/`** for personal/project-only workflows where you want short skill names like `/deploy`. Use a **plugin** when you want versioning, namespaced commands like `/my-plugin:deploy`, or distribution to a team or community.
...
**What it is.** A JSON file at `.claude-plugin/plugin.json` that identifies the plugin, declares its version, and (optionally) overrides default component paths. It is the only file that belongs in `.claude-plugin/` — every other directory (skills, agents, hooks, etc.) lives at the plugin root.

**When to use it.** Always when distributing — even though the manifest is technically optional (Claude Code can auto-discover components and use the directory name), you need it as soon as you want a stable name, version pinning, marketplace metadata, or non-default component paths.
...
**Full schema fields.** `name` (required, kebab-case, becomes the namespace prefix), `version` (semver — omit to fall back to git commit SHA), `description`, `author`, `homepage`, `repository`, `license`, `keywords`, plus path overrides: `skills`, `commands`, `agents`, `hooks`, `mcpServers`, `lspServers`, `monitors`, `outputStyles`, `themes`, plus `userConfig`, `channels`, `dependencies`.
...
- [`anthropics/skills`](https://github.com/anthropics/skills) — Anthropic's public Agent Skills repository (PDF processing, document analysis, etc.).
...
**Live examples.**
- [`wshobson/agents`](https://github.com/wshobson/agents) — 80 plugins / 185 specialized agents (architect-review, security-auditor, kubernetes-architect, etc.) using a tiered Opus/Sonnet/Haiku model strategy.
...
**When to use it.** Auto-format on `PostToolUse`, block dangerous commands on `PreToolUse`, inject project context on `SessionStart`, run tests after edits, scan for secrets before commits, react to file changes with `FileChanged`, etc.
...
**What it is.** JSON files under `themes/` that ship color themes selectable from `/theme`. Each theme declares a `base` preset (dark/light) and a sparse `overrides` map of color tokens.

**When to use it.** Distribute branded color schemes, accessibility-focused themes, or popular community palettes (Dracula, Solarized, etc.). Plugin themes are read-only; users press Ctrl+E in `/theme` to copy one to `~/.claude/themes/` for editing.
...
**What it is.** Markdown files under `output-styles/` that change how Claude formats its responses (e.g., terse, verbose, educational tone).

**When to use it.** When the plugin wants to enforce a particular conversational style — security review tone, teaching tone, terminal-only output, etc.
...
**When to use it.** Ship CLI helpers your skills and hooks call — formatters, linters, custom wrappers, internal scripts. Avoids hard-coding `${CLAUDE_PLUGIN_ROOT}/bin/foo` everywhere.
...
**When to use it.** When the plugin enables Claude to receive messages from outside the terminal — chat-bridge plugins, on-call paging, etc.
...
**When to use it.** Split a large plugin into reusable building blocks (e.g., a shared "secrets-vault" plugin that auth plugins depend on). `claude plugin prune` cleans up auto-installed dependencies no longer needed.
...
| `claude plugin validate`             | Lint `plugin.json`, frontmatter, `hooks.json` for schema/syntax errors.   |
...
- [ivan-magda/claude-code-plugin-template](https://github.com/ivan-magda/claude-code-plugin-template) — marketplace template with validation/CI.
