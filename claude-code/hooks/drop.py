...
When the user types `drop A`, `drop A,C`, or `drop all` as the entire prompt,
this hook deletes the matching entries from the most recent batch and blocks
the prompt with a one-line confirmation — no Claude turn consumed.

Other "drop" forms ("drop it", "drop foo-slug") aren't matched and fall through
to Claude's normal handling per the add skill.
"""
...
def main():
    ...
    # Guard: don't intercept "drop it" — let Claude handle it conversationally.
    ...
    if not deleted:
        # Nothing actually deleted — pass through to Claude so they can ask why.
        return 0
