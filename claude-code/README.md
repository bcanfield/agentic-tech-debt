...
- **`/debt-ops:review`** — audit and triage the registry in one pass. Flags entries whose hotspot file no longer exists as `stale` (droppable with `drop A,B,…`), deprioritizes files unchanged in 90+ days as `cold`, and ranks the rest by change frequency and risk to surface the top 3 to pay down next. Run when the registry feels heavy. See [ADR 0007](../doc/adr/0007-registry-review-skill.md).
