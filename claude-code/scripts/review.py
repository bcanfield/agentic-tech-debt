...
Audit signals (deterministic, Git-as-oracle):
...
- Writes letter mappings for stale entries to current-turn.txt so the user
  can drop them with the existing `drop A,B,C` UX (ADR 0004 + drop.py).

The output is a one-screen, three-bucket report intended to be the entire
user-facing reply when /debt-ops:review fires.
"""
...
def print_report(stale_map, top, cold, active_kept_count):
    ...
    if active_kept_count:
        lines.append(f"kept ({active_kept_count}) — active, lower-ranked; rerun /debt-ops:review after paydown")
