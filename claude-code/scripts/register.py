...
Replaces the older Write-tool-driven flow in the `add` skill. Writing through
this helper keeps the agent's mid-turn footprint to one line — the helper's
own stdout — and assigns a short letter (A, B, C, ...) per entry so the user
can later say `drop A` (handled by drop.py UserPromptSubmit hook) instead of
typing the full slug.
"""
