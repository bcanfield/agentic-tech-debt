...
# /debt-ops:paydown — gated autonomous paydown
...
- **Interactive** (a developer is in the session): escalations go through the
  `AskUserQuestion` tool — clean structured choices, no browser, no file round-trip.
- **Headless** (CI, `claude -p`, no human mid-run): you cannot block on a person.
...
If unsure, assume interactive and use `AskUserQuestion`.

## Get the ranked list

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/review/scripts/review.py --top 20
```

`review.py` already does the deterministic half — staleness audit + churn × quadrant
ranking. You don't re-derive it. Propose its stale-flagged entries for `drop` first
(don't pay down dead code), then classify the survivors below.
...
- the body offers **multiple credible options** ("options at payoff time: …") —
  choosing among them is the human's call and per CLAUDE.md becomes an ADR, not an
  autonomous edit;
...
5. **Fresh-context self-review.** Spawn a `code-review` sub-agent (via Task) on the
   diff — *always*, not just for risky fixes. In supervised paydown a human reads each
   diff; here nobody does, so writer/reviewer separation is mandatory. If it flags a
   correctness problem, revert and escalate.
...
**Interactive — `AskUserQuestion`.** One question per escalated entry (or batch a few),
...
- **Proposed drops (N):** stale entries `review.py` flagged — `drop A,B,C`.
