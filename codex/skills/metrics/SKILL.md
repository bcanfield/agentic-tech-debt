---
name: metrics
description: Print a debt-ops health summary from the metrics log — registration rate, hook feedback action rate, ADR creation, AI-authored share. Use when the user asks for "debt-ops metrics", "debt health", "registry stats", or invokes $metrics. Read-only — never writes the log.
---
