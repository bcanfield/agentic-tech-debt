...
def disciplines_text(adr_dir, registry_dir):
    return (
        ...
        '`TODO`/`FIXME`/`HACK`/`XXX` marker — register via the `$add` skill immediately. Test: would a '
        ...
        'comment, not an ADR. If the ADR introduces deliberate debt, also invoke `$add` so the '
